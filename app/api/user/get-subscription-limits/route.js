...
export async function GET(req) {
    try {
        ...
        const userSubscription = await db
            ...
            .where(
                and(
                    eq(UsersSubscriptions.user_id, parseInt(userId)),
        ...
        if (!userSubscription.length) {
            ...
            const freeTier = await db
                ...
                .where(ilike(SubscriptionsTypes.name, 'Free'))
                .limit(1);
            ...
            const freeTierLimits = await db
                .select()
                .from(SubscriptionLimits)
                .where(eq(SubscriptionLimits.subscription_type_id, freeTier[0].id))
                .limit(1);
        ...
        const limits = await db
            .select()
            .from(SubscriptionLimits)
            .where(eq(SubscriptionLimits.subscription_type_id, userSubscription[0].subscription_type_id))
            .limit(1);
