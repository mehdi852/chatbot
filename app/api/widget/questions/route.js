...
export async function GET(request) {
    ...
    try {
        ...
        const questions = await db
            ...
            .where(and(eq(WidgetQuestions.website_id, parseInt(websiteId)), eq(WidgetQuestions.is_active, true)))
        ...
        return NextResponse.json({ error: 'Failed to load widget questions' }, { status: 500 });
    }
}
...
export async function POST(request) {
    ...
    try {
        ...
        const maxOrder = existingQuestions.length > 0 ? Math.max(...existingQuestions.map((q) => q.order_index)) : -1;
        ...
        return NextResponse.json({ error: 'Failed to create widget question' }, { status: 500 });
    }
}
    ...
    try {
        ...
        for (let i = 0; i < questions.length; i++) {
            ...
            await db
                ...
                .where(and(eq(WidgetQuestions.id, question.id), eq(WidgetQuestions.website_id, parseInt(websiteId))));
        }
        ...
        return NextResponse.json({ error: 'Failed to update widget questions order' }, { status: 500 });
    }
}
...
export async function DELETE(request) {
    ...
    try {
        ...
        // Delete the question
        await db.delete(WidgetQuestions).where(and(eq(WidgetQuestions.id, parseInt(questionId)), eq(WidgetQuestions.website_id, parseInt(websiteId))));
        ...
        return NextResponse.json({ error: 'Failed to delete widget question' }, { status: 500 });
    }
}
